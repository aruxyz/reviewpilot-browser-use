...
try:
    ...
    print(f"  Already set or failed: {e}")

print()
print("=== 2. Prepare files on main via Git Data API ===")
main_ref = repo.get_git_ref("heads/main")
main_sha = main_ref.object.sha
base_tree = repo.get_git_commit(main_sha).tree
...
elements = [
    InputGitTreeElement(
        path=".github/workflows/reviewpilot.yml",
        mode="100644",
        type="blob",
        content=workflow_yaml,
    ),
    InputGitTreeElement(
        path=".reviewpilot.yml",
        mode="100644",
        type="blob",
        content=config_yaml,
    ),
...
print("  Creating tree with workflow + config...")
new_tree = repo.create_git_tree(elements, base_tree=base_tree)
print(f"  Tree SHA: {new_tree.sha}")

print("  Creating commit...")
new_commit = repo.create_git_commit(
    "Add ReviewPilot workflow and config",
    new_tree,
...
main_ref.update(new_commit.sha)
print("  main updated")
